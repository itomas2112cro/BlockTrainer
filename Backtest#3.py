...
#%% Z-Score
sma = B.moving_average(btc_price_weekly[:,3], 26)
std = B.std(btc_price_weekly[:,3],26)
Z = (btc_price_weekly[:,3] - sma)/std
Z = Z[start:end+1].copy()
plt.plot(Z); plt.show()

#%% EMA_Slope
ema_slope = np.insert(np.diff(ema),0, np.nan).copy()

#%% Max drawdown
drawdown = B.DD(btc_price_weekly[start:end+1,3]).copy()
plt.plot(drawdown); plt.title("Drawdown"); plt.show()

drawdown_signal = np.clip(1+drawdown, 0.5, 2)
plt.plot(drawdown_signal); plt.title("Drawdown Signal"); plt.show()

#%% Define indicators
rsi_indicator = ((RSI >= 80)&(Z>=1.5)).copy()
ema_indicator = (btc_price_weekly[start:end+1,3] < ema).copy()
ath_indicator1 = ((btc_price_weekly[start:end+1,3] > ATH_weekly) & (ema_slope>0) & (RSI < 75)).copy()
ath_indicator2 = (btc_price_weekly[start:end+1,3] > ATH_weekly).copy()

ath_indicator = ath_indicator2.copy()

## Define how much to invest under EMA
drawdown_signal[ema_indicator]*250
...
investment[ema_indicator] = drawdown_signal[ema_indicator]*250
investment[rsi_indicator] = 62.5
investment[(ath_indicator1)&(investment<250)] = 250
...
categories = pd.Series(parity_investment).apply(classify_investment)

# Count occurrences by category
counts = categories.value_counts().reindex(["62$", "125$", "250$", ">250$"], fill_value=0)
...
np.round(np.array([55,31,416,55])/557*100,2)
